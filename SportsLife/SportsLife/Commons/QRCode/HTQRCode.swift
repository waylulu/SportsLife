...
class HTQRCode: UIView {
    ...
    func addDescView(){
        ...
        labelDesc = UILabel()
        labelDesc.frame = CGRect(x: 0, y: 45 + 64, width: self.frame.width, height: 30)
        labelDesc.text = "将二维码放入框内,即可自动扫描"
        labelDesc.textColor = UIColor.white
        labelDesc.textAlignment = NSTextAlignment.center
        labelDesc.font = UIFont.systemFont(ofSize: 15)
    ...
    func addLine(){
        ...
        imgLine.image = UIImage(named: "QRCode_Line")
    ...
    func addCornerGreenLine(ctx:CGContext, rect:CGRect){
        ...
        ctx.setStrokeColor(red: 255/255.0, green: 86/255.0, blue: 85/255.0, alpha: 1) //红色
        ...
        ctx.addLine(to: CGPoint.init(x: rect.origin.x + rect.size.width - lineHeight, y: rect.origin.y + rect.size.height))
        ctx.strokePath()
